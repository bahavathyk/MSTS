# -*- coding: utf-8 -*-
"""
Created on Thu Mar  3 12:13:50 2022
...
from MSFunctions.MSFunctions import Calc_MIScore, find_dtw_dist, knn_clfcrossvalxv, knn_clfcrossvalxv_precomp, Create_input, Cal_numer, FindMerit_Score_knn, Create_input_selected, Findaccuracy, FindCombsaccuracy
from MSFunctions.Load_Data import FindUEAXy, list_files
from MSFunctions.feat_feat_MINorm import knnMI_FF
from MSFunctions.feat_class_MI import knnMI_FC
...
for i in range(0,n):
	...
	y_prediction = cross_val_predict(knn_clf, dist[i],y_true, cv=3)
	y_pred.insert(i,i,y_prediction)
	 
	start_MS_acc_1 = time.process_time()
	y_acc = cross_val_score(knn_clf, dist[i],y_true, cv=3, scoring = 'accuracy')
...
k = 10
MI_knn = knnMI_FF(dist, k, n, y_true, m)
MI_y_knn = knnMI_FC(X_full, y_true, k, n, m)
...
writefile = open("Results/"+Dataset_Name+"_knn.txt", "w+")


for  i in range(0,n-1):
	merit_score, combs = FindMerit_Score_knn(MI_y_knn,var_no, n, sel_comb, MI_knn, X_full)
...
plt.savefig("Results/"+Dataset_Name+"_knn.png") 
...
writefile = open("Results/"+Dataset_Name+"_knn.txt", "a")
var_no = 2
for  i in range(0,n-1):
...
plt.figure(dpi = 300)
#RS
#MI_knn = [0.989032, 0.698945, 0.902695, 0.776057, 0.924479, 0.921674]

#JV
MI_knn = [1.20498,1.09161, 1.2989, 0.87754, 0.929323, 1.06191, 1.16338, 1.04806, 1.31691, 1.15433, 0.805469, 1.08281]
plt.scatter(MI_y, MI_knn)
plt.title('JapaneseVowels: Single Feature class correlation score')
plt.xlabel("Our Technique")
plt.ylabel("k-nn Technique")
corr = pearsonr(MI_y[0].values, MI_knn)
plt.text(0.3, 0.9, "r = " + str(round(corr[0],2)), fontsize=12, style='italic')
'''